...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Real, deliberate simplification, per explicit instruction: multilingual support (Hindi,
    // Hinglish) is genuinely deferred until there's budget for a provider that can do it
    // properly without the training/data-sharing concerns found when researching Sarvam.
    // English-only for now -- and for English specifically, forcing the language directly is
    // both simpler and more reliable than the auto-detect-and-validate approach that existed
    // purely to handle a second language: English is Whisper's single most-trained language by
    // a wide margin, and there's no longer a second plausible language to confuse it with a
    // real detection failure being retried against.
